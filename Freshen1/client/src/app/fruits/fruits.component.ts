...
@Component({
  ...
  styleUrls: ['./fruits.component.css'], // Note the plural `styleUrls`
})
export class FruitsComponent implements OnInit {
  fruits: any[] = []; // Array to hold only fruits
  ...
  ngOnInit(): void {
    this.productService.getProducts().subscribe({
      next: (response) => {
        // Filter products to include only those with type 'fruit'
        this.fruits = response.data.filter((product: any) => product.type === 'Fruit');
      },
      error: (error) => {
